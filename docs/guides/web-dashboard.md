...
The dashboard ships in all release binaries: [GitHub Releases](https://github.com/agent-of-empires/agent-of-empires/releases), the [quick install script](../installation.md#quick-install-recommended), and Homebrew (`brew install aoe`). Just run `boa serve`.
...
```bash
boa serve                       # Localhost only (safe, default)
boa serve --remote              # Remote over HTTPS (Tailscale Funnel, else Cloudflare quick tunnel)
boa serve --host 0.0.0.0        # LAN/VPN access (HTTP, requires VPN)
boa serve --daemon              # Run in background
boa serve --open                # Open the URL in the default browser when ready
boa serve --remote --read-only  # Monitor without terminal input
```

The server prints a URL with an auth token:

```
boa web dashboard running at:
...
In `--remote` mode the auth token rotates every 4 hours, so a URL captured at startup eventually stops working. Use `boa url` against a running daemon (exits non-zero if none is running):

```bash
boa url               # Primary URL with the live token
boa url --all         # Every labeled URL (Tailscale / LAN / localhost), tab-separated
boa url --token-only  # Just the token (for scripted login)
```
...
`--remote` is the recommended way to reach the dashboard from your phone. BOA picks a transport automatically, in this order. For the end-to-end phone setup, see [Remote Access from Your Phone](remote-phone-access.md).

### 1. Tailscale Funnel (preferred when available)

If `tailscale` is on PATH and logged in, BOA runs `tailscale funnel --bg --yes <port>` and exposes the dashboard at your stable `https://<machine>.<tailnet>.ts.net` URL. No domain, no Cloudflare account, no rotating URLs. **This is the only option where a PWA installed on your phone keeps working across server restarts** (the URL is stable).

One-time setup (BOA surfaces the fix if a gate is missing):
...
If port 443 already has a non-loopback Funnel service on this node, BOA refuses to start rather than replace it (a stale loopback config from a prior BOA run is overwritten cleanly). Clear the conflict with `tailscale funnel reset` (the Error dialog offers `[R]`), or pass `--no-tailscale` to use Cloudflare.
...
# Add a CNAME: boa.example.com -> <tunnel-id>.cfargotunnel.com
boa serve --remote --tunnel-name my-tunnel --tunnel-url boa.example.com
```

### 3. Cloudflare quick tunnel (fallback)

Zero-config but the URL rotates on every restart. Fine for one-off sessions, **bad for installed PWAs** (the home-screen app is bound to its install URL, so a restart means delete-and-reinstall). BOA prints a notice when it falls back here.
...
When TLS is terminated by an external proxy (Traefik, nginx, Caddy) forwarding to `boa serve` on loopback (often through an SSH reverse tunnel), use `--behind-proxy` so cookies carry `; Secure` and the rate limiter keys by the real client IP:
...
The upstream must set `X-Forwarded-For` (or `cf-connecting-ip`); BOA reads the last value as the client IP. The trust check fires only when the socket peer is loopback, so a misconfigured upstream that lets requests reach BOA directly cannot spoof the IP.
...
- **Session persistence**: login sessions are persisted to an owner-only `login_sessions.toml` in the app dir, so signed-in devices survive a `boa serve` restart instead of being re-prompted for the passphrase. A passphrase change drops every persisted session; set `auth.persist_sessions = false` to force re-authentication on every restart.
...
- **Localhost** (`boa serve`): same security as the TUI.
- **Remote via tunnel** (`boa serve --remote`): encrypted via HTTPS. Recommended for phone access.
- **Over Tailscale/WireGuard** (`boa serve --host 0.0.0.0`): the VPN encrypts traffic.
- **Behind a reverse proxy** (`--auth=passphrase --behind-proxy`): TLS terminated upstream; passphrase is the only human gate.
- **Read-only** (`boa serve --remote --read-only`): monitor without input.

### Dangerous (blocked)

- `boa serve --host 0.0.0.0` on public WiFi without a VPN: traffic is unencrypted HTTP.
- `boa serve --auth=none --host 0.0.0.0` (or `--no-auth --host 0.0.0.0`): refuses to start without `--behind-proxy`.
- `boa serve --auth=none --remote` or `--auth=passphrase --remote`: refuses to start.

## Installing as a PWA

The dashboard installs as a Progressive Web App for an app-like, standalone window:

- **macOS (Chrome)**: three-dot menu > "Install Band of Agents".
- **macOS (Safari)**: File > Add to Dock.
- **iOS**: Share > Add to Home Screen.
- **Android (Chrome)**: "Add to Home Screen" prompt or install banner.

The PWA needs the server running; use `--daemon` to keep it up (`boa serve --stop` to stop). For a stable URL that survives restarts, install from a Tailscale Funnel or named-Cloudflare URL, not a quick tunnel.
...
`Ctrl-C` on a foreground server, or `boa serve --stop` against a daemon, both exit within ~5 seconds even with open tabs. Live clients receive a `1001` ("going away") close frame and reconnect once a fresh server is running.
